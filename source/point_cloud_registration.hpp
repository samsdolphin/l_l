...
{
public:
	std::string path_name = "/home/sam/loop_test/Loam_livox_pts";
	std::string file_name = std::string(path_name).append("/test");

	ceres::LinearSolverType slover_type = ceres::SPARSE_NORMAL_CHOLESKY; // SPARSE_NORMAL_CHOLESKY | DENSE_QR
		...
		*(m_logger_timer->get_ostream())<< m_timer->toc_string("Query points for match") << std::endl;
			...
			m_current_frame_index > m_mapping_init_accumulate_frames)
		{
			...
			*(m_logger_timer->get_ostream()) << m_timer->toc_string("Build kdtree") << std::endl;
			...
			//fprintf(fp, "R_last: %f %f %f %f\n", m_q_w_last.w(), m_q_w_last.x(), m_q_w_last.y(), m_q_w_last.z());
			//fprintf(fp, "t_last: %f %f %f\n", m_t_w_last(0), m_t_w_last(1), m_t_w_last(2));
			cout<<m_current_frame_index<<endl;
			...
			for (iterCount = 0; iterCount < m_para_icp_max_iterations; iterCount++)
			{
				...
				for (int i = 0; i < laser_corner_pt_num; i++)
				{
					...
					if (m_kdtree_corner_from_map.nearestKSearch(pointSel, line_search_num, m_point_search_Idx, m_point_search_sq_dis) != line_search_num) // return # of neighbors found
						continue;
				...
				for (int i = 0; i < laser_surface_pt_num; i++)
				{
					...
					if (m_point_search_sq_dis[plane_search_num - 1] < m_maximum_dis_plane_for_match)
					{
						...
						if (IF_PLANE_FEATURE_CHECK)
						{
							...
							if ((saes.eigenvalues()[2] > 3 * saes.eigenvalues()[0]) && (saes.eigenvalues()[2] < 10 * saes.eigenvalues()[1]))
								planeValid = true;
							else
								planeValid = false;
						}
			...
			//fprintf(fp, "R_incre: %f %f %f %f\n", q_w_incre.w(), q_w_incre.x(), q_w_incre.y(), q_w_incre.z());
			//fprintf(fp, "t_incre: %f %f %f\n", t_w_incre(0), t_w_incre(1), t_w_incre(2));
			fclose(fp);
			//cout << "total constraints number: " << cons_num << std::endl;
			//cout << "Dump to file OK, current_frame_index: " << m_current_frame_index << std::endl;
			...
			*(m_logger_common->get_ostream()) << "Iteration time: " << iterCount << endl;
			...
			minimize_cost, m_inlier_final_threshold, summary.num_residual_blocks, corner_avail_num, surf_avail_num, angular_diff, m_t_diff);
									   ...
									   pcl::PointCloud<PointType>::Ptr laserCloudSurfStack)
	{
		...
		return find_out_incremental_transfrom(in_laser_cloud_corner_from_map, in_laser_cloud_surf_from_map,
											  m_kdtree_corner_from_map, m_kdtree_surf_from_map,
	...
	unsigned int pointcloudAssociateToMap(pcl::PointCloud<PointType> const &pc_in, pcl::PointCloud<PointType> &pc_out, int if_undistore = 0)
	{
		unsigned int points_size = pc_in.points.size();
		pc_out.points.resize(points_size);
	...
	unsigned int pointcloudAssociateTbeoMapped(pcl::PointCloud<PointType> const &pc_in, pcl::PointCloud<PointType> &pc_out)
	{
		unsigned int points_size = pc_in.points.size();
		pc_out.points.resize(points_size);

		for (unsigned int i = 0; i < points_size; i++)
			pointAssociateTobeMapped(&pc_in.points[i], &pc_out.points[i]);
