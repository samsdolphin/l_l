...
namespace Ceres_pose_graph_3d
{
    struct Pose3D
    {
        ...
        Pose3D() = default;
        Pose3D(Eigen::Quaterniond &in_q, Eigen::Vector3d &in_t) : q(in_q), p(in_t) {};
    ...
    inline std::istream &operator>>(std::istream &input, Pose3D &pose)
    {
        ...
        pose.q.normalize(); // Normalize the quaternion to account for precision loss due to serialization.
        return input;
    }
    ...
    public:
        ...
        static bool ReadVertex(std::ifstream *infile, std::map<int, Pose, std::less<int>, Allocator> *poses)
        {
            int id;
            Pose pose;
            *infile >> id >> pose;
        ...
        static void ReadConstraint(std::ifstream *infile, std::vector<Constraint, Allocator> *constraints)
        {
    ...
    void pose_graph_optimization(MapOfPoses &poses, VectorOfConstraints &constraints)
    {
        ...
        std::cout << summary.IsSolutionUsable() << endl;
    }
...
public:
    ...
    static std::vector<Eigen::Matrix<T1, 3, 1>> refinePoints(std::vector<Eigen::Matrix<T1, 3, 1>> &raw_pt_vec,
                                                             const Eigen::Matrix<T2, 3, 3> &R_mat_ori,
                                                             Eigen::Matrix<T2, 3, 1> &t_vec_ori,
                                                             const Eigen::Matrix<T2, 3, 3> &R_mat_opm,
                                                             Eigen::Matrix<T2, 3, 1> &t_vec_opm)
    {
        std::vector<Eigen::Matrix<float, 3, 1>> opm_pt_vec;
        opm_pt_vec.resize(raw_pt_vec.size());
        Eigen::Matrix<T1, 3, 3> R_aff = (R_mat_opm * R_mat_ori.transpose()).template cast<T1>();
        Eigen::Matrix<T1, 3, 1> T1_aff = (R_aff.template cast<T2>() * (-t_vec_ori) + t_vec_opm).template cast<T1>();

        for (size_t i = 0; i < raw_pt_vec.size(); i++)
            opm_pt_vec[i] = R_aff * raw_pt_vec[i] + T1_aff;
                        ...
                        int if_save = 1)
    {
        ...
        std::vector<Eigen::Matrix<float, 3, 1>> raw_pts_vec, opm_pts_vec;
        for (auto it = pose3d_map_oir.begin(); it != pose3d_map_oir.end(); it++)
        {
            ...
            raw_pts_vec = PCL_TOOLS::pcl_pts_to_eigen_pts<float, _PointType>(map_idx_pc.find(id)->second.makeShared()) ;
            opm_pts_vec = refinePoints(raw_pts_vec, pose_ori.q.toRotationMatrix(), pose_ori.p, pose_opm.q.toRotationMatrix(), pose_opm.p);
            pcl::PointCloud<_PointType> pcl_pts = PCL_TOOLS::eigen_pt_to_pcl_pointcloud<_PointType>(opm_pts_vec);
            if (if_save)
            {
                opm_pts_vec = PCL_TOOLS::pcl_pts_to_eigen_pts<float, _PointType>(pcl_pts.makeShared());
                PointCloudMap<float> pt_cell_temp;
                pt_cell_temp.set_resolution(1.0);
                pt_cell_temp.set_point_cloud(opm_pts_vec);
    ...
    void refine_mapping(std::string &path_name, int if_save = 1)
    {
        ...
        PointCloudMap<float> pt_cell_map;
        PointCloudMap<float> pt_cell_full;
        ...
        for (auto it = filename_map.begin(); it != filename_map.end(); it++)
        {
            ...
            std::vector<Eigen::Matrix<float, 3, 1>> raw_pts_vec, opm_pts_vec;
            raw_pts_vec = pt_cell_map.load_pts_from_file(file_name);
            map_idx_pc.insert(std::make_pair(id, PCL_TOOLS::eigen_pt_to_pcl_pointcloud<pcl::PointXYZ>(raw_pts_vec)));
        }
