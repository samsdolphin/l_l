...
struct ceres_icp_point2point_mb
{
    ...
    bool operator()(const T *_q, const T *_t, T *residual) const
    {
        ...
        Eigen::Matrix<T, 3, 1> pt{ T(current_pt_(0)), T(current_pt_(1)), T(current_pt_(2)) };
...
{
    Eigen::Matrix<_T, 3, 1> current_pt_;
    Eigen::Matrix<_T, 3, 1> target_line_a_, target_line_b_;
    ...
    ceres_icp_point2line_mb(const Eigen::Matrix<_T, 3, 1> &current_pt,
                          ...
                          const _T motion_blur_s = 1.0,
                          Eigen::Matrix<_T, 4, 1> q_s = Eigen::Matrix<_T, 4, 1>(1, 0, 0, 0),
                          Eigen::Matrix<_T, 3, 1> t_s = Eigen::Matrix<_T, 3, 1>(0, 0, 0)) : current_pt_(current_pt), target_line_a_(target_line_a),
                                                                                               target_line_b_(target_line_b),
                                                                                               m_motion_blur_s(motion_blur_s),
                                                                                               m_q_last(q_s),
                                                                                               m_t_last(t_s)
    {
    ...
    bool operator()(const T *_q, const T *_t, T *residual) const
    {
        ...
        Eigen::Matrix<T, 3, 1> tar_line_pt_a = target_line_a_.template cast<T>();
...
{
    Eigen::Matrix<_T, 3, 1> current_pt_;
    Eigen::Matrix<_T, 3, 1> target_line_a_, target_line_b_, m_target_line_c;
    Eigen::Matrix<_T, 3, 1> unit_vec_ab, m_unit_vec_ac, m_unit_vec_n;
    _T m_motion_blur_s;
    _T weigh;
    Eigen::Matrix<_T, 4, 1> m_q_last;
    Eigen::Matrix<_T, 3, 1> m_t_last;
...

    {
        ...
        m_unit_vec_n = unit_vec_ab.cross(m_unit_vec_ac);
        weigh = 1.0;
    };
